...
void install_beta(GtkWidget *widget, gpointer adobe)
{
  ...
  system("kdesudo 'rm -f /var/lib/flashplugin-nonfree/npwrapper.libflashplayer.so' ");
  system("rm -rf $HOME/.wine/dosdevices/c:/windows/system32/Macromed/Flash");
  system("wget --progress=bar:force 'http://fpdownload.macromedia.com/get/flashplayer/current/install_flash_player_10_linux.tar.gz' -O/dev/null 2>&1 | zenity --title='Downloading...' --progress --auto-close --auto-kill");
  system("tar xvfz install_flash_player_10_linux.tar.gz");
  system("kdesudo 'mv libflashplayer.so /usr/lib/mozilla/plugins/libflashplayer.so'");
  system("rm install_flash_player_10_linux.tar.gz");
  system("zenity --info --text 'Flash Player 10.1 Installed Successfully'");
}
